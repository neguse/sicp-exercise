...
; test lambda
(test-is
  (myeval '((lambda (x) (+ x 1)) 1) the-global-environment)
  2)
...
; test begin
(test-is (myeval '(begin 1 2 3) the-global-environment) 3)
(test-is (myeval '(begin
					(define x 0)
					(set! x 1)
					(set! x 2)
					x) the-global-environment) 2)

(test-done)

; (driver-loop)

; うーん、実行結果はどっちも変わらないんだよなぁ
; 本文の方は、実行の直列化がanalyzeのタイミングでなされるのだけど
; Alyssa版はそうでないので、コントロールフローが確定しないのが問題なのでは?
; いまんところ大差ないけど、機械語みたいなのに落とすことを考えると
; たぶんだめなんだろうなあという感じで…
