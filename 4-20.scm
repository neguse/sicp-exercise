...
(define (letrec->nested-lets exp)
  ...
  (define (set-vars-exp vars exps)
	(if (null? vars)
	  ...
	  (cons (list 'set! (car vars) (car exps))
			(set-vars-exp (cdr vars) (cdr exps)))))
  (define (make-let vars exps bodies)
	(append
	  (list 'let (unassined-vars-exp vars))
	  (append (set-vars-exp vars exps) bodies)))
  (let
	((vars (let-parameters-vars (let-parameters exp)))
	 (exps (let-parameters-exps (let-parameters exp)))
	 (bodies (let-body exp)))
	(make-let vars exps bodies)))

(define (install-letrec)
  (define (eval-letrec exp env)
	(myeval (letrec->nested-lets exp) env))
...
; まあ、実装はコピペで作れるので、別にという感じ。

; Louisさんのは…4.19と同じ話じゃないかなあ。

; 関数を定義するときに、関数のネストにするべきかどうか悩む。
; ネストにするとスコープが切れるので「この関数内でしか使ってないんだな」ということがわかりやすくなる。
; スコープが広いと、「この関数どこだよ」ってことになりやすいし、依存関係が思ったよりあって変更時に想定してないところまで影響でたりする。
; いっぽう、共通コードみたいなのがあったときにもとの定義から切り出して移動する必要が出てくるのでちょっと面倒。
; 再帰とかする以外の目的でネストしている場合、別にネストする必要性あまりないじゃんというのはある。

; あと、スコープ的に外に出てないとテストコードから直接呼べないという問題もある。
; こっちはこっちでけっこう問題。

; 高い抽象を作るのって、素人には難しい気がしていて
; 最初はコピペとかでいいのでスコープが切れてた方がいいんじゃないか。
; どういう抽象があると完結に書けるかというのは、ある程度コードベースが大きくならないと
; 見えてこないのじゃないかなぁ。

; 共通部分のコード、使われれば使われるほど実装を変更しづらくなるので
; 難しいなぁ。
