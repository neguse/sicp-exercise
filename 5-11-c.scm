  ...
  (let ((contents '*unassigned*))
	(define (dispatch message)
	  (cond ((eq? message 'get) contents)
			((eq? message 'set)
			 (lambda (value) (display (list name value)) (newline) (set! contents value)))
			(else
...
(define (push k stack value)
  (display 'push)
  (display (stack k 'push))
  (newline)
  ((stack k 'push) value))

(define (make-mapped-stack register-names)
  (let ((m '()))
	(define (lookup k)
	  (assoc k m))
    (define (initialize ks)
	  (cond
		((not (null? ks))
		 (set! m (cons (cons (car ks) (make-stack)) m))
		 (initialize (cdr ks)))))
	(define (dispatch k message)
	  (let ((s (lookup k)))
		(if s
		  ((cdr s) message)
...
(define (make-new-machine register-names)
  (let ((pc (make-register 'pc))
		...
		(stack (make-mapped-stack register-names))
...
(define (duplicate-label? label labels)
  (cond
	((null? labels) #f)
	((eq? (caar labels) label) #t)
	(else (duplicate-label? label (cdr labels)))))
	...
	(extract-labels (cdr text)
					(lambda (insts labels)
					  (let ((next-inst (car text)))
						(if (symbol? next-inst)
						  (if (duplicate-label? next-inst labels)
...
(define (make-save inst machine stack pc)
  (let ((regname (stack-inst-reg-name inst))
	...
	(lambda ()
	  (push regname stack (get-contents reg))
	  (advance-pc pc))))
...
(define (make-restore inst machine stack pc)
  (let ((regname (stack-inst-reg-name inst))
		(reg (get-register machine
						   (stack-inst-reg-name inst))))
	(lambda ()
	  (set-contents! reg (pop regname stack))    
	  (advance-pc pc))))
