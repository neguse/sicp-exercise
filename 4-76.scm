...
; Racketのstream apiにあわせる
(define the-empty-stream empty-stream)
(define stream-null? stream-empty?)
(define (cons-stream a d) (stream-cons a d))
(define (stream-car l) (stream-first l))
(define (stream-cdr l) (stream-rest l))
...
(define (conjoin-merge-stream s1 s2)
  (stream-flatmap
	(lambda (f1)
	  (stream-filter
		...
		(stream-map
		  (lambda (f2)
			(conjoin-merge-frame f1 f2))
		  s2)))
	s1))


(define (conjoin-merge-frame f1 f2)
  (cond
	((null? f1) f2)
	(else
	  (let ((var (caar f1))
		...
		(let ((extension (extend-if-possible var val f2)))
		  (if (equal? extension 'failed)
			'failed
			(conjoin-merge-frame (cdr f1) extension)))))))
...
; …とおもったのだけど、計算量はかわらないっぽいし
; 出力がかわっちゃうので、微妙ですね。

; 試しにandの中ふやして実行時間くらべてみたけど
; あんまりかわんないですね。

; あと、ユニフィケーションに似てるって書いてあるけど
; そこまで似てないですよね。
