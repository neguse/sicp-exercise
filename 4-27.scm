...
; 実行するまえに考えてみる。
; countの値はid関数の呼び出しによってインクリメントされるわけだけど
; 遅延評価になるので増えないんじゃないかな。
...
; ??? 全然違う…
; まず、最初のcountが1になってるのが謎ですね。
; …あっ、wのdefineは関数じゃなくて変数なんですか。
; ということは、wをdefineした時点でインクリメントされてるというのは
; (id (id 10))のうち1回分がここで行われてて、
; wのサンクをforceしたタイミングでもう1回分が行われるということでしょうか。
